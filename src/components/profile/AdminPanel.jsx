...
export default function AdminPanel({ session, profile }) {
  ...
  const deleteGroup = async (id) => {
    ...
    if (error) { console.error('[AdminPanel] Delete group failed:', error.message); alert('Failed to delete group.'); return; }
    await loadGroups();
  };
  ...
  const promoteUser = async (userId, groupId) => {
    ...
    if (error) { console.error('[AdminPanel] Promote user failed:', error.message); return; }
    await loadUsers();
  };
  ...
  const removeUser = async (userId, groupId) => {
    ...
    if (error) { console.error('[AdminPanel] Remove user failed:', error.message); return; }
    await loadUsers();
  };
  ...
  const postBriefing = async () => {
    ...
    if (error) { console.error('[AdminPanel] Post briefing failed:', error.message); setPostingBriefing(false); alert('Failed to post briefing.'); return; }
  ...
  const postNewsBriefing = async () => {
    ...
    if (error) { console.error('[AdminPanel] News briefing failed:', error.message); setPostingNews(false); alert('Failed to post briefing.'); return; }
    setSelectedNews([]);
