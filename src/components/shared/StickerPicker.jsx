// ============================================
// UPTIKALERTS — StickerPicker.jsx
// Trading-themed sticker picker — one tap to send
// Compact, low-friction, works in chat + trash talk
// ============================================

import { useState, useRef, useEffect } from 'react';
import { createPortal } from 'react-dom';

// Curated trading stickers — small set = fast decisions = stickiness
...
/**
 * StickerButton — the small toggle that opens the picker
 * @param {Function} onSend - called with sticker object when user picks one
 * @param {string} size - 'sm' (trash talk) or 'md' (main chat)
 */
export default function StickerPicker({ onSend, size = 'md' }) {
  ...
  useEffect(() => {
    if (open && btnRef.current) {
      ...
      // Right-align popup to button, clamped within viewport
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {/* Portal renders outside the overflow:hidden app container */}
      {open && createPortal(
        <>
...
/**
 * StickerMessage — renders a sticker as a compact card in chat
 * Emoji + label underneath, with a subtle background
 */
export function StickerMessage({ stickerId, size = 'md' }) {
  ...
  return (
    <div style={{
      ...
      padding: sm ? '4px 8px 3px' : '8px 14px 6px',
...
/**
 * Utility: check if a message is a sticker-only message
 * Sticker messages are stored as "sticker:rocket" etc.
 */
export function isSticker(text) {
  return text?.startsWith('sticker:');
}

export function getStickerId(text) {
